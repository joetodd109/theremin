...
static uint32_t spi_dma_current;
...
spi_i2s_configure_dma1_str(int16_t * const src1, int16_t * const src2, uint16_t nbytes)
{
    DMA_Stream_TypeDef cfg = {
        .CR = (1u << DMA_CR_TCIE_Pos) /* Transfer complete interrupt enabled. */
            ...
            |(0u << DMA_CR_DBM_Pos)   /* Double buffer mode enabled */
            |(0u << DMA_CR_CHSEL_Pos),      /* Channel Selection. */
        .NDTR = nbytes,
        .PAR = (uint32_t) & SPI3->DR,
        .FCR = (1u << DMA_FCR_DMDIS_Pos)  /* FIFO enabled */
            |(1u << DMA_FCR_FEIE_Pos)     /* FIFO error interrupt enabled */
    };

    // cfg.M0AR = (uint32_t) src1;
    // cfg.M1AR = (uint32_t) src1;
    // dma_init_dma1_chx(5u, (DMA_Stream_TypeDef const *) &cfg);

    cfg.M0AR = (uint32_t) src2;
    cfg.M1AR = (uint32_t) src2;
